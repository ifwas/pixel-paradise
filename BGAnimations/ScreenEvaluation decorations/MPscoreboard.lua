...
local function scoreitem(pn, i)
	local t = Def.ActorFrame {
		...
		Def.Quad {
			InitCommand = function(self)
				self:xy(framex, framey + ((i - 1) * spacing) - 3):zoomto(frameWidth, 31):halign(0):valign(0):diffuse(
					color("#ffffff")
		...
		Def.Quad {
			UpdateNetScoreCommand = function(self)
				self:xy(framex, framey + ((i - 1) * spacing) - 3):zoomto(3, 31):halign(0):valign(0):diffuse(
