...
local PLEASEEEEMINAAAAAAAINEEEDTHISX = 35
...
local function rankingButton(i)
	local t = Def.ActorFrame {
		InitCommand = function(self)
			self:xy(PLEASEEEEMINAAAAAAAINEEEDTHISX + (i - 1) * rankingTitleSpacing, rankingY * 1.15)
		end,
		UIElements.QuadButton(1, 1) .. {
			InitCommand = function(self)
...
local function percentButton()
	local t = Def.ActorFrame {
		InitCommand = function(self)
			self:xy(rankingX + capWideScale(6,2.5) * rankingTitleSpacing, 24 * 0.75):valign(1)
		end,
		UIElements.QuadButton(1, 1) .. {
			InitCommand = function(self)
				self:zoomto(rankingTitleSpacing, 26):diffuse(getMainColor("frames")):diffusealpha(0.2)
...
local function littlebits(i)
	local t = Def.ActorFrame {
		...
		LoadFont("Common Large") .. {
			InitCommand = function(self)
				self:xy(45,tdujujdjshgfihjdfsg * i):maxwidth(170 * 2):halign(0):zoom(0.4)
			end,
			SetCommand = function(self)
				self:settext(ms.SkillSetsTranslated[i] .. ":")
			end,
		...
		LoadFont("Common Large") .. {
			InitCommand = function(self)
				self:xy(210, txtDist * i):halign(0):zoom(0.4)
			end,
			SetCommand = function(self)
				local rating = 0
				if not showOnline then
					...
					self:GetParent():x(capWideScale(28,45))
					self:x(210)
				else
...
local profilebuttons = Def.ActorFrame {
	...
	UIElements.TextToolTip(1, 1, "Common Large") .. {
		InitCommand = function(self)
			self:xy(frameX + frameWidth * 5.3/7, frameHeight + 4):zoom(0.3):diffuse(getMainColor("positive"))
			self:settext(translated_info["RefreshSongs"])
		end,
		MouseOverCommand = function(self)
			self:diffusealpha(hoverAlpha)
		end,
