--good luck trying to make this 4:3 compatible
...
t[#t + 1] = Def.Sprite {
	InitCommand = function(self)
		self:xy(frameX + (frameWidth / 2), frameY):zoom(0.45):valign(0)
		self:Load(THEME:GetPathG("","spr/tab/tagTitleBox")):SetTextureFiltering(false)
		self:diffuse(getMainColor("positive"))
	end
}
t[#t + 1] = LoadFont("Common Normal") .. {
	InitCommand = function(self)
		self:xy(frameX + (frameWidth / 2), frameY + 10):zoom(0.65)
	...
	local t = Def.ActorFrame {
		...
		Def.ActorFrame {
			...
			UIElements.SpriteButton(1, 1, nil) .. {
				InitCommand = function(self)
					self:xy(-6, 20):zoom(0.42):halign(0):valign(1)
					self:Load(THEME:GetPathG("","spr/tab/actualTagbox")):SetTextureFiltering(false)
		            self:diffuse(getMainColor("positive"))
				end,
				UpdateTagsMessageCommand = function(self)
					...
					if tagFunction == 1 then
						if song and curTag and ptags[curTag][steps:GetChartKey()] then
							...
							self:diffuse(getMainColor("negative")):diffusealpha(0.35)
						end
			...
			LoadFont("Common Large") .. {
				Name = "Text",
				InitCommand = function(self)
					self:xy(75, 5):maxwidth(frameWidth + 25)
				end,
				UpdateTagsMessageCommand = function(self)
					self:zoom(fontScale)
					if playertags[i + ((currenttagpage - 1) * tagsperpage)] then
...
local function funcButton(i)
	local t = Def.ActorFrame {
		...
		LoadFont("Common Large") .. {
			InitCommand = function(self)
				self:y(16):halign(0.5):diffuse(getMainColor("positive")):maxwidth((frameWidth / 3)):maxheight(22):zoom(0.75)
			end,
			BeginCommand = function(self)
				self:settext(fawa[i])
			end,
...
r[#r + 1] = Def.ActorFrame {
	...
	LoadFont("Common Large") .. {
		InitCommand = function(self)
			self:x(30):halign(0):zoom(fontScale)
		end,
		BORPBORPNORFNORFcCommand = function(self)
