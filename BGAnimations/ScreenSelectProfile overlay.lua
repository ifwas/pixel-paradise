...
t[#t + 1] = Def.ActorFrame {
	...
	children = {
		Def.ActorFrame {
			Name = "P1Frame",
			InitCommand = function(self)
				self:xy(1200, SCREEN_CENTER_Y)
			end,
			OnCommand = function(self)
				self:sleep(0.1):decelerate(0.4):x(SCREEN_CENTER_X)
			end,
			OffCommand = function(self)
				self:decelerate(0.5):x(SCREEN_LEFT - 500)
			end,
...
t[#t + 1] = LoadFont("Common Large") .. {
	InitCommand = function(self)
		self:xy(capWideScale(20,20), 25):halign(0):valign(1):zoom(0.3):diffuse(getMainColor("positive"))
		self:settextf("%s:", translated_info["Title"])
	end
}
t[#t+1] = LoadFont("Common Large") .. {
	InitCommand = function(self)
		self:xy(40, SCREEN_HEIGHT-26)
