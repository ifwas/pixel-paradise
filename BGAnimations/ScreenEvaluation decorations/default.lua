	...
	LoadFont("Common Large") .. {
		...
		SetCommand = function(self)
			if GAMESTATE:GetCurrentSong():GetDisplaySubTitle() == "" then
			   ...
			   self:settextf("\"%s\"",GAMESTATE:GetCurrentSong():GetDisplaySubTitle())
		    end
		end,
	},
}
...
local function scoreBoard(pn, position)
	...
	local t = Def.ActorFrame {
		...
		Def.Sprite {
		    InitCommand = function(self)
			    self:xy(frameRadarX, frameRadarY):zoom(0.45):halign(0):valign(0)
			    self:Load(THEME:GetPathG("","spr/evaluation/radarbox")):SetTextureFiltering(false)
			    self:diffuse(ColorMultiplier(getMainColor("highlight"),1.5))
		end
	    },
		Def.Quad {
	...
	local function radarEntry(i)
		return Def.ActorFrame {
			...
			LoadFont("Common Normal") .. {
				InitCommand = function(self)
					self:xy( frameRadarX + 10, frameRadarY + 3 + 13 * i)
					self:zoom(0.45)
					self:halign(0)
			...
			LoadFont("Common Normal") .. {
				InitCommand = function(self)
					self:xy(frameRadarX + 100, frameRadarY + 3 + 13 * i)
					self:zoom(0.45)
					self:halign(1)
				end,
				BeginCommand = function(self)
	...
	if devianceTable ~= nil then
		...
		local tzoom = lines == 5 and 0.45 or 0.3
		local ySpacing = lines == 5 and 12 or 8.5
		local function statsLine(i)
			return Def.ActorFrame {
				...
				LoadFont("Common Normal") .. {
					Name = "StatText",
					InitCommand = function(self)
						self:xy( frameRadarX + 120, frameRadarY + 5 + ySpacing * i)
						self:zoom(tzoom)
						self:halign(0)
				...
				LoadFont("Common Normal") .. {
					...
					SetCommand = function(self, params)
						...
						if i < 4 then
							self:xy( frameRadarX + 220, frameRadarY + 5 + ySpacing * i)
							self:zoom(tzoom)
							self:halign(1)
